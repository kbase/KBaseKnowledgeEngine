...
public class KBaseKnowledgeEngineServerTest {
    ...
    private static KBaseKnowledgeEngineServer impl = null;
    ...
    public static void init() throws Exception {
        ...
        config = ini.get("KBaseKnowledgeEngine");
        // Token validation
        ...
        JsonServerSyslog.setStaticMlogFile(new File(config.get("scratch"), "test.log")
            .getAbsolutePath());
        impl = new KBaseKnowledgeEngineServer();
    }
    
    private static String getWsName() throws Exception {
        if (wsName == null) {
            long suffix = System.currentTimeMillis();
            wsName = "test_KBaseKnowledgeEngine_" + suffix;
    ...
    private static RpcContext getContext() {
        return new RpcContext().withProvenance(Arrays.asList(new ProvenanceAction()
            .withService("KBaseKnowledgeEngine").withMethod("please_never_use_it_in_production")
